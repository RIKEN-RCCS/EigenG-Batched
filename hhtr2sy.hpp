...
//  const int BLK_J = (std::is_same<T,float>::value) ? 4 : 3;
  ...
  for (int i=ii+1; i<=n; i+=BLK_I) {
    ...
    for(int IK=IIKK%4;IK<IIKK;IK+=4) {
      ...
      sum4_on_warp(G[K0][I0],G[K1][I1],G[K2][I2],G[K3][I3]);
    }}
#else
    for(int I=0;I<BLK_I;I++) {
      for(int K=0;K<=I;K++) {
    ...
    for (int j=1; j<=m%BLK_J; j++) {
      ...
      for (int k=myid; k<=i+(BLK_I-1)-1; k+=WARP_GPU_SIZE) {
        ...
        _if_(II&0x2) sum2_on_warp(s[0],s[1]);
        _if_(II&0x1) sum_on_warp(s[II-1]);
      for(int I=II;I<BLK_I;I+=4) {
        sum4_on_warp(s[I],s[I+1],s[I+2],s[I+3]);
      }}
      for(int I=0;I<BLK_I;I++) {
      for(int K=0;K<I;K++) {
        s[I] += s[K] * G[K][I];
    ...
    for (int j=1+m%BLK_J; j<=m; j+=BLK_J) {
      ...
      { int II=(BLK_I*BLK_J)%4;
        _if_(II&0x2) sum2_on_warp(s[0][0],s[1%BLK_I][1/BLK_I]);
        _if_(II&0x1) sum_on_warp(s[(II-1)%BLK_I][(II-1)/BLK_I]);
      for(int IJ=II;IJ<BLK_I*BLK_J;IJ+=4) {
        ...
        sum4_on_warp(s[I0][J0],s[I1][J1],s[I2][J2],s[I3][J3]);
      }}
#else
      for(int J=0;J<BLK_J;J++) {
      for(int I=0;I<BLK_I%4;I++) {
        ...
        sum4_on_warp(s[I][J],s[I+1][J],s[I+2][J],s[I+3][J]);
      }}
#endif
      for(int J=0;J<BLK_J;J++) {
      for(int I=0;I<BLK_I;I++) {
